...
pub const JIT = struct {
    ...
    fn tryTemplateBased(self: *Self, module: *Module, func_idx: u32, func: Module.Function) !?CompiledFunction {
        ...
        if (has_loop and arithmetic_density > 10 and has_bitwise and has_shifts) {
            const compiled = try self.compileCryptoLoopTemplate(func_idx);
            return compiled;
        }

        // Template 2: Memory-intensive operations (like Git object storage)
        if (has_memory_ops and memory_density > 10 and has_arithmetic) {
        ...
        // Template 9: Compile everything! Maximum performance mode
        const compiled = try self.compileGenericOptimizedTemplate(func_idx, func);
        return compiled;
    }
