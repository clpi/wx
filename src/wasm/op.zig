...
pub const Op = union(@import("op/type.zig").OpType) {
    ...
    fn _matchSlow(o: u8) ?Op {
        return switch (o) {
            0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x0B => .{ .control = .match(o) },
            ...
            0x3F, 0x40 => .{ .memory = .match(o) },
            0x28, 0x2C, 0x2D, 0x2E, 0x2F, 0x3A, 0x3B, 0x41, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B,
            0x4C, 0x4D, 0x4E, 0x4F, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAE, 0xAF, 0xB0,
            0xB1, 0xB2, 0xB3, 0xB4, 0x67, 0x68, 0x69, 0x6A, 0xB5, 0xB6, 0xB7, 0xB8, 0xB9, 0xBA, 0xBB, 0xBC,
            0xBD, 0xBE, 0xBF, 0xC0, 0xC1, 0x36, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F, 0x70,
            0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78 => .{ .i32 = .match(o) },
            0x83, 0x29, 0x42, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5A,
            0x7A, 0x3D, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35,
            0x3E, 0xC2, 0xC3, 0xC4, 0x37,
            0x3C, 0x7B, 0x7C, 0x7D, 0x7E, 0x7F, 0x80, 0x81, 0x82, 0x84, 0x85, 0x86,
            0x87, 0x88, 0x89 => .{ .i64 = .match(o) },
            0x79, 0x8A, 0x5B, 0x5C, 0x5D, 0x5E, 0x5F, 0x60, 0x61, 0x62, 0x63, 0x2A, 0x43, 0x64,
            0x65, 0x66, 0x8B, 0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95,
            0x96, 0x97, 0x98 => .{ .f32 = .match(o) },
            0x99, 0x9A, 0x9B, 0x9C, 0x9D, 0x9E, 0x9F, 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5,
            0xA6, 0x2B, 0x39, 0x44 => .{ .f64 = .match(o) },
            0xD0, 0xD1, 0xD2, 0xD3, 0xD5 => .{ .ref = .match(o) },
            else => return null,
        };
    }

    // Table-based dispatch can be introduced later if desired.
    ...
    pub const Control = enum(u32) {
        ...
        pub inline fn match(op: u32) @This() {
            return switch (op) {
                0x00 => .@"unreachable",
                0x01 => .nop,
                0x02 => .block,
...
test "op handle" {
    ...
    // Added smoke checks for common arithmetic and memory opcodes
    try std.testing.expect(Op.match(0x6A).? == .i32); // i32.add
    try std.testing.expect(Op.match(0x6B).? == .i32); // i32.sub
    try std.testing.expect(Op.match(0x6C).? == .i32); // i32.mul
    try std.testing.expect(Op.match(0x7C).? == .i64); // i64.add
    try std.testing.expect(Op.match(0x7D).? == .i64); // i64.sub
    try std.testing.expect(Op.match(0x7E).? == .i64); // i64.mul
    try std.testing.expect(Op.match(0x2A).? == .f32); // f32.load
    try std.testing.expect(Op.match(0x2B).? == .f64); // f64.load
    try std.testing.expect(Op.match(0x28).? == .i32); // i32.load
    try std.testing.expect(Op.match(0x29).? == .i64); // i64.load
    try std.testing.expect(Op.match(0x36).? == .i32); // i32.store
    try std.testing.expect(Op.match(0x37).? == .i64); // i64.store
    try std.testing.expect(Op.match(0x38).? == .f32); // f32.store
    try std.testing.expect(Op.match(0x39).? == .f64); // f64.store
}
